...
for plate in Path(args.input_dir).glob('*.txt'):
    ...
    del parsed_df, melted_df, semifinal_df,semifinal_df2,tmp, raw_input_df
    gc.collect()
    ...
    Tm_df.to_csv(output_dir_string+"/Final_results_plate_"+str(plate_count)+".txt",sep="\t",index=False)
    final_curves.to_csv(output_dir_string+"/Final_curves_plate_"+str(plate_count)+".txt",sep="\t",index=False)
    plate_report.to_csv(output_dir_string+"/Plate_report_plate_"+str(plate_count)+".txt",sep="\t",index=False)
    well_error_count_df.to_csv(output_dir_string+"/Potential_problems_plate_"+str(plate_count)+".txt",sep="\t",index=False)

Tm_files = [file for file in os.listdir(output_dir_string) if (file.endswith(".txt"))&file.startswith("Final_results_plate_")]
curve_files = [file for file in os.listdir(output_dir_string) if (file.endswith(".txt"))&file.startswith("Final_curves_plate_")]
plate_report_files = [file for file in os.listdir(output_dir_string) if (file.endswith(".txt"))&file.startswith("Plate_report_plate_")]
well_error_count_files = [file for file in os.listdir(output_dir_string) if (file.endswith(".txt"))&file.startswith("Potential_problems_plate_")]

input_count = len([file for file in os.listdir(args.input_dir) if file.endswith(".txt")])
...
if len(Tm_files) < input_count:
    pass
else:
    from DSF_functions import concatenate_files
    concatenate_files(Tm_files, output_dir_string, 'Final_results.txt')
    concatenate_files(curve_files, output_dir_string, 'Final_curves.txt')
    concatenate_files(plate_report_files, output_dir_string, 'Plate_report.txt')
    concatenate_files(well_error_count_files, output_dir_string, 'Potential_problems.txt')
    size = get_folder_size(output_dir_string)
    size_mb = size / (1024 * 1024)
    print("Size of output directory for "+str(plate_count)+" plates is: "+str(size_mb))
    for filename in glob.glob(output_dir_string+"/Final_results_plate_*"):
        os.remove(filename)
    for filename in glob.glob(output_dir_string+"/Final_curves_plate_*"):
        os.remove(filename)
    for filename in glob.glob(output_dir_string+"/Plate_report_plate_*"):
        os.remove(filename)
